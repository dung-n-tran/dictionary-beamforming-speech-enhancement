...
compute_null_controlling_tf_beamformers_2, compute_steering_vectors_circular
from tqdm import tnrange, tqdm

class DictionaryLearningBeamformerCircular(object):
    ...
    def _compute_source_steering_vectors(self):
        ...
        for i_source_angle, source_angle in enumerate(self.source_angles):
            v = compute_steering_vectors_circular(self.array_geometry, 
                    self.sampling_frequency, self.stft_params, 
                    source_angle["azimuth"])
            source_steering_vectors.append(v)
        return source_steering_vectors
    
    def _compute_steering_vectors(self):
        return compute_steering_vectors_circular(self.array_geometry,
                    self.sampling_frequency, self.stft_params,
                    self.angle_grid["azimuth"])
...
#                     print(training_data[i_configuration][0].shape, tf_sample_covariance_batch.shape, batch_indices)
                    ...
                    null_steering_vectors = compute_steering_vectors_circular(
                        self.array_geometry, self.sampling_frequency,
                        self.stft_params, np.unique(training_data[i_configuration][1]["azimuth"])
                    )
                    null_steering_vectors = np.transpose(null_steering_vectors, (0, 2, 1))
            
                    w = compute_null_controlling_tf_beamformers_2(
                            self.source_steering_vectors[i_source][:, 0, :], null_steering_vectors, 
    ...
    def _compute_null_angle_ranges(self, null_azimuths, desired_null_width):
        azimuth_ranges = []
        for null_azimuth in null_azimuths:
            azimuth_ranges.append(
                np.arange(null_azimuth - desired_null_width/2,
                          null_azimuth + desired_null_width/2, 0.1))
        return np.concatenate(azimuth_ranges)
